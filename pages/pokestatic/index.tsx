...
export default function MyList2({ initialData }: any) {
  ...
  useEffect(() => {
    async function getData() {
      try {
        ...
        let response = await fetch(`https://pokeapi.co/api/v2/location/?limit=10&offset=${page}`);
  ...
  return (
    <div className="text-white">
      <div>
        <h4 className="mb-4 text-2xl font-extrabold leading-none tracking-tight text-white text-center">Static</h4>
        {isLoading ? (
...
export async function getStaticProps() {
  ...
  try {
    const res = await fetch(`https://pokeapi.co/api/v2/location/?limit=10&offset=${INITIAL_PAGE_NUMBER}`);
